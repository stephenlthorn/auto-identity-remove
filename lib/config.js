...
// Falls back to the .bak file when the primary file is missing (recovery from
// a mid-rename kill between the bak-rename and the tmp-rename in an old save).
function resetState() {
  const target = getStatePath();
  const bak = target + '.bak';
  let fresh;
  if (fs.existsSync(target)) {
    fresh = JSON.parse(fs.readFileSync(target, 'utf8'));
  } else if (fs.existsSync(bak)) {
    fresh = JSON.parse(fs.readFileSync(bak, 'utf8'));
  } else {
    fresh = { optOuts: {} };
  }
...
function saveState() {
  ...
  // Single atomic rename: on POSIX this is atomic - the old target is never
  // absent between the rename and any prior backup step (unlike the two-rename
  // approach that created a window where both tmp and target could be absent).
  fs.renameSync(tmp, target);
  // Rotate a backup AFTER the new state is safely in place. Use copyFileSync
  // (not rename) so target is never absent if the copy is interrupted.
