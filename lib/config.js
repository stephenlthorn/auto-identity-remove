...
function recordSuccess(brokerName, detail = '') {
  ...
  // A fresh success resolves any re-verification quarantine on this entry.
  delete state.optOuts[brokerName].needsReverify;
  saveState();
}
...
function shouldSkip(brokerName) {
  ...
  // Quarantined entries (needsReverify, set by lib/reverify.js) are never
  // skipped: the recorded outcome is untrusted, so the broker is always
  // re-attempted until a fresh success clears the flag.
  if (entry.needsReverify) return null;
...
/**
 * Return the label that identifies `person` inside a composite state key.
 *
 * `person.stateLabel` wins when set: two household members can share a legal
 * name (e.g. a personal and a work profile for the same person), and without a
 * distinct label both would collapse onto one "Broker|First Last" key - one
 * person's recorded opt-out would suppress the other's submission. Configs
 * without stateLabel keep the historical "First Last" label.
 *
 * @param {object} person
 * @returns {string}
 */
function personLabel(person) {
  if (!person) return '';
  const label = String(person.stateLabel || '').trim();
 ...
 * Multi-person mode (totalPersons > 1 AND person provided): returns
 * "BrokerName|<personLabel>" where the label is person.stateLabel when set,
 * else "First Last".
 *
...
function stateKey(brokerName, person, totalPersons) {
  ...
  return `${brokerName}|${personLabel(person)}`;
}
