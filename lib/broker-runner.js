...
const { recordSuccess, recordPendingConfirmation, shouldSkip } = require('./config');
...
async function processBrokerWithPerson(context, broker, person) {
  ...
  try {
    ...
    await withRetry(() => page.goto(broker.optOutUrl, { waitUntil: 'domcontentloaded', timeout: 15000 }));
