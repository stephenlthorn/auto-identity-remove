...
async function processBrokerWithPerson(context, broker, person) {
  ...
  try {
    ...
    if (broker.method === 'search-form' && broker.searchUrl) {
      // A navigation failure here is NOT "not listed". findListingUrl throws
      // when the search page fails to load (DNS, timeout, bot-block, crash);
      // collapsing that to null used to report 'notFound' for a listing that
      // was never searched, which is exactly the false "you're not on this
      // site" verdict that hides a live listing. Only a completed search that
      // found nothing may report notFound.
      let searchError = null;
      try {
        listingUrl = await findListingUrl(page, broker);
      } catch (err) {
        searchError = err;
      }
      if (searchError) {
        const msg = searchError.message?.includes('Timeout')
          ? 'Timeout'
          : (searchError.message || 'unknown').slice(0, 80);
        logResult(broker.name, 'error', `listing search failed: ${msg}`);
