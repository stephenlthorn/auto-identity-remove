...
function matchBrokers(serpHostnames, brokerHostnames) {
  const brokerSet = new Set(brokerHostnames);
  const seen = new Set();
  const result = [];
  for (const h of serpHostnames) {
    if (brokerSet.has(h) && !seen.has(h)) {
      seen.add(h);
      result.push(h);
    }
  }
...
/**
 * Build a map from bare hostname -> broker name from the brokers array.
 * Uses optOutUrl first, falls back to searchUrl.
 *
 * @param {object[]} brokers
 * @returns {Map<string, string>}  hostname -> broker name
 */
  ...
  for (const person of persons) {
    ...
    for (let engineIdx = 0; engineIdx < ENGINES.length; engineIdx++) {
      ...
      for (const result of serpResults) {
        ...
        if (!host) continue;
        const brokerName = brokerIndex.get(host);
 ...
 *   results: Array<{ broker: string, ranks: { ddg: number|null, bing: number|null, google: number|null } }>,
 *   blocked: string[],
 * }} SerpScanSummary
 */
