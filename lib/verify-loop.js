...
/**
 * Return the state key for a (broker, person) pair.
 * Single-person usage: persons has exactly one entry with no name disambiguation
 * needed, so key = broker.name.  Multi-person: key = "broker.name|First Last".
 */
function stateKey(brokerName, person, totalPersons) {
  if (totalPersons <= 1) return brokerName;
