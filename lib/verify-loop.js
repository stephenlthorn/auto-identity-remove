...
async function runVerify(context, brokers, persons, opts = {}) {
  ...
  for (const broker of brokers) {
    ...
    for (const person of persons) {
      ...
      // Quarantined entries (needsReverify, set by lib/reverify.js) bypass the
      // lastSuccess requirement and the 7-day wait: the recorded outcome is
      // untrusted, so the listing is re-searched immediately.
      const flagged = !!(record && record.needsReverify);

      // ── Gate 1: must have a recorded lastSuccess (unless quarantined) ─────
      if (!record || (!record.lastSuccess && !flagged)) {
        skipped.push({ broker: broker.name, person, reason: 'no recorded opt-out submission' });
        continue;
      }

      const lastSuccessMs = record.lastSuccess ? new Date(record.lastSuccess).getTime() : NaN;
      const daysSinceSuccess = (now.getTime() - lastSuccessMs) / (1000 * 60 * 60 * 24);

      // ── Gate 2: must be >= VERIFY_AFTER_DAYS since submission (unless quarantined)
      if (!flagged && daysSinceSuccess < VERIFY_AFTER_DAYS) {
      ...
      // A real verification result resolves the quarantine flag (if any).
      const { needsReverify: _dropFlag, ...recordWithoutFlag } = record;
