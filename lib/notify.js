...
function sendText(message, notifyCfg) {
  if (!notifyCfg?.textTo) return;
...
/**
 * POST summary to a webhook URL (ntfy.sh / Slack / Discord compatible).
 * Uses Node 18+ global fetch — no external dependency.
 * @param {string} webhookUrl
 * @param {string} text
 */
async function _webhookPost(webhookUrl, text) {
  try {
