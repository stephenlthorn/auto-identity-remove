...
async function sendOptOutEmails(brokers, cfg, _platform, opts = {}) {
  ...
  for (const broker of emailBrokers) {
    // Allowlist: the user explicitly wants to STAY listed on this broker, so
    // no opt-out email is sent. Same guard the explicit-broker path applies.
    if (isAllowlisted(broker.name, cfg)) {
      loggerMod.logResult(broker.name, 'allowlisted', 'on allowlist - keeping listing, no opt-out email sent');
      continue;
    }

    for (const person of persons) {
      // Recheck guard is keyed per (broker, person): in multi-person mode the
      // success is recorded under the composite stateKey, so a bare-name check
      // here would never suppress a resend - and one person's fresh opt-out
      // must not suppress a different person's pending one.
      const key = stateKey(broker.name, person, personCount);
      if (configMod.lastOptOutDaysAgo(key) < configMod.RECHECK_DAYS) {
