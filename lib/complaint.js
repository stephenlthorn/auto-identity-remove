...
const { personLabel } = require('./config');
...
function findOverdue(state, opts = {}) {
  ...
  // Keyed by personLabel (stateLabel when set, else "First Last") so composite
  // state keys built by config.stateKey map back to the right person - two
  // household members can share a legal name and must not resolve to the same
  // complainant.
